...
object KmeansByAllColumns {
  ...
  def processKmeans(centroidNumber: Integer) {
    ...
    */
    ...
    val initRDD = reduceToCentroid( mapToCentroid( datasetRDD,
      dataAvgList.toList ) )
  ...
  /*
  Se mapea cada elemento ( x ) => ( conjunto de dimensiones del centroide
  al cual pertenece, conjunto de dimensiones del elemento x )
  */
  def mapToCentroid( rdd : RDD[List[Double]], dataAvgList : List[List[Double]] )
  : RDD[Tuple2[String, List[Double]]] =
    rdd.map( x => {
      val minDistance =
  ...
  */
  ...
  /*
  Se reMapea cada elemento ( tuple2(centroide, Lista de sus elementos ) )
  => ( conjunto de dimensiones del centroide al cual pertenece,
  conjunto de dimensiones del elemento x )
  */
  def reMapToCentroid(rdd : RDD[Tuple2[List[Double], List[List[Double]]]])
  : RDD[Tuple2[String, List[Double]]] = {
    ...
    ( Conjunto de dimensiones del centroide al que pertenece,
    conjunto dimensiones de cada elemento de la lista )
    */
    rdd.flatMap( myTuple =>
      for (value <- myTuple._2) yield ( myTuple._1, value ) )

    /*
    Se reMapea cada elemento ( x ) => ( Conjunto de dimensiones
    del centroide al cual pertenece, conjunto de valores de x )
    */
    .map( x => {
  ...
  */
  def reduceToCentroid(rdd : RDD[Tuple2[String, List[Double]]]) :
  RDD[Tuple2[List[Double], List[List[Double]]]] =
    rdd
